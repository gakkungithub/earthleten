    ...
    return (
        <>
            ...
            {wikiCards.map((wc, index) => (
                <Link key={index} href={`/playercoach/profile/${wc.id}`} className={`col-span-1 w-27 h-36 border-2 ${colorClassMap[wc.bgColor].bg} hover:bg-opacity-50 mx-auto rounded`}>
                <ul className={`${colorClassMap[wc.textColor].text} text-center rounded`}>
                    ...
                    <li className="relative group text-sm text-center">
                        ...
                        <div className={`absolute left-1/2 -translate-x-1/2 top-full mt-1 hidden group-hover:block ${colorClassMap[wc.textColor].bg} ${colorClassMap[wc.bgColor].text} border p-2 rounded shadow-lg z-10 w-fit`}>
                            {wc.sports.join(', ')}
                        </div>
                    </li>
                    <li className="relative group text-sm text-center">
